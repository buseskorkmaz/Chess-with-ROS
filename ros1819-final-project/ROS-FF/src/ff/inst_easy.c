...

{
   ...
   */
  //Ali, to skip removing preconditions
//  for ( i = 0; i < gnum_easy_operators; i++ ) {
//    o = geasy_operators[i];
...

{
  ...
  if ( curr_parameter == lnum_multiply_parameters ) {
   ...
   //     printf("11111 NAME IS : %s \n", lo->opr->name);
      tmp->inst_table[i] = lo->inst_table[i];
    }
    for ( unsigned j = 0; j < lo->num_vars; j++ ) {
  //      printf( " 1111111 IS : %d \n " ,j);
  //      printf( " %s \n ", gconstants[j] );
  //       printf( " %s \n ", gconstants[ tmp->inst_table[j] ] );
    }
