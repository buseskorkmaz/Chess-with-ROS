...

{
    ...
    for ( i = 0; i < o->num_removed_vars; i++ ) {
        m = o->removed_vars[i];
        printf("\nx%d (%s) of type %s, type constraint ", m, o->opr->var_names[m],
               gtype_names[o->opr->var_types[m]]);
...

{
    ...
    for ( i = 0; i < o->opr->num_vars; i++ ) {
        printf("\nx%d = %s of type ", i, gconstants[o->inst_table[i]]);
        print_type( o->opr->var_types[i] );
    }
    ...
    for ( e = o->effects; e; e = e->next ) {
        ...
        for ( i = 0; i < e->num_vars; i++ ) {
            printf("\nx%d of type %s",
                   o->opr->num_vars + i, gtype_names[e->var_types[i]]);
        }
        printf("\nConditions\n");
        print_Wff( e->conditions, 0 );
        printf("\nEffect Literals");
...

{
    ...
    if ( !a->norm_operator &&
        ...
        unsigned int s=strlen(a->name);
        for ( i = 0; i < a->num_name_vars; i++ ) {
           s = s + strlen(gconstants[a->name_inst_table[i]]) + 1;//adding one for a blank space
        }
        ...
        for ( i = 0; i < a->num_name_vars; i++ ) {
            ...
            strcat(str, " ");
            strcat(str, gconstants[a->name_inst_table[i]]);

        }
//        printf("\n%s %d\n", str, indexArray);


        strcpy(solutionPlan[indexArray], str);
        //solutionPlan[indexArray]= str;
