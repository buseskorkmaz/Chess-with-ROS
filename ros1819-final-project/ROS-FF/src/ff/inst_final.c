...

{
  ...
  while ( !fixpoint ) {
    ...
    //ALI, to include some required predicates.
 //   checkFact = gpredicates[lp];

 //  printf("prreddicaatteee :%s: \n", checkFact);


//    if ( !strcmp(checkFact,"PLACEMENT") || !strcmp(checkFact,"PLACEMENTST") ||
//         !strcmp(checkFact,"HASACC") || !strcmp(checkFact,"GRASP") || !strcmp(checkFact,"ISCRIT") ||
//         !strcmp(checkFact,"HASACCHOLDING") )
//    {
...
//            for ( unsigned j = 0; j < garity[lp]; j++ ) {
//                grelevant_facts[gnum_relevant_facts].args[j] = largs[j];
//                //         printf("gnum_relevant_facts : %d : %d\n", largs[j], gnum_relevant_facts);
//                //         printf("arrggssss : %s \n", gconstants[(grelevant_facts[gnum_relevant_facts]).args[j] ]);
//            }
//            //   lindex[lp][fact_adress()] = counter;
//            gnum_relevant_facts++;
...
//    }
  ...
  //       printf("prreddicaatteee : %d : %s \n", lp, gpredicates[lp]);
      ...
      adr = fact_adress();
      //Ali, I think disabling this condition add all the facts regarding the side effect !
      if ( !lpos[lp][adr] ) {
        ...
        grelevant_facts[gnum_relevant_facts].predicate = lp;
  //     printf("iiiii : %d : %d\n", lp, gnum_relevant_facts);
 //       printf("prreddicaatteee : %s \n", gpredicates[(grelevant_facts[gnum_relevant_facts]).predicate]);
        for ( j = 0; j < garity[lp]; j++ ) {
          grelevant_facts[gnum_relevant_facts].args[j] = largs[j];
 //         printf("gnum_relevant_facts : %d : %d\n", largs[j], gnum_relevant_facts);
 //         printf("arrggssss : %s \n", gconstants[(grelevant_facts[gnum_relevant_facts]).args[j] ]);
    ...
    tmp->inst_table[i] = t1->inst_table[i];
      ...
      tmp->name = no->opr->name;
      tmp->num_name_vars = no->opr->number_of_real_params;
    ...
    for ( j = 0; j < pae->num_adds; j++ ) {
      ...
      for ( j = 0; j < pa->opr->num_vars; j++ ) {
    tmp->inst_table[j] = pa->inst_table[j];
      }
      tmp->name = pa->opr->name;
      tmp->num_name_vars = pa->opr->number_of_real_params;
  ...
  if ( gcmd_line.display_info == 118 ) {
    ...
    if ( ( a->norm_operator &&
           a->norm_operator->opr !=  goperators[i] ) ||
         ( a->pseudo_action &&
           a->pseudo_action->opr !=  goperators[i] ) ) {
      continue;
    }
    printf("\ntemplate: ");
...

{
  ...
  for ( i = 0; i < o->opr->number_of_real_params; i++ ) {
...

{
  ...
  for ( i = 0; i < pa->opr->number_of_real_params; i++ ) {
