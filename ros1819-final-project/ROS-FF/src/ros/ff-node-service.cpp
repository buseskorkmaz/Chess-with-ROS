...
#include "src/ff/memory.h"
#include "src/ff/output.h"

#include "src/ff/parse.h"

#include "src/ff/inst_pre.h"
#include "src/ff/inst_easy.h"
#include "src/ff/inst_hard.h"
#include "src/ff/inst_final.h"

#include "src/ff/orderings.h"

#include "src/ff/relax.h"
#include "src/ff/search.h"
}
...
std::vector <std::string> solutionPl;
...

{
   ...
   */
    ...
    if ( found_plan ) {
        ...
        //std::string plan1(solutionPlan[0]);
        //std::cout<<"plan1"<<" "<<std::string(solutionPlan[0])<<std::endl;
        //std::cout<<"plan2"<<" "<<std::string(solutionPlan[1])<<std::endl;
        //printf("\n%d\n", indexArray);

        for(unsigned int ip=0; ip<indexArray; ip++ ) {
            std::string plan(solutionPlan[ip]);
            //std::cout<<plan<<" "<<ip<<std::endl;
            solutionPl.push_back(plan);
            //std::cout<<solutionPl.back()<<std::endl;
        }
...
void print_official_result( void )
{
    ...
    sprintf( name, "%s%s.ff", gcmd_line.path, gcmd_line.fct_file_name );
...

{
    ...
    //        strncpy( gcmd_line.ops_file_name, argv[0], MAX_LENGTH );
    //        strncpy( gcmd_line.fct_file_name, argv[1], MAX_LENGTH );
...
bool srvPlan(ff::Plan::Request &req, ff::Plan::Response &res)
{
    char *prob = new char[req.problem.length() + 1];
    std::strcpy(prob, req.problem.c_str());

    char *act = new char[req.domain.length() + 1];
    std::strcpy(act, req.domain.c_str());

    char *argv[5] = {"node", "-f", prob, "-o", act};
...
{
    ros::init(argc, argv, "ff_node_service");
    ros::NodeHandle n;

    ROS_INFO("Starting FF ROS Node");

    //    char *argv2[5] = {"node", "-f", "/home/users/aliakbar.akbari/catkin_assembly/src/FF-TaskPlanner/ff-domains/block_world", "-o", "/home/users/aliakbar.akbari/catkin_assembly/src/FF-TaskPlanner/ff-domains/block_world.pddl"};
    //    plan(5,argv2);

    ros::Rate loop_rate(10.0); //Choose the loop rate
    ros::AsyncSpinner spinner(4); //Choose how many threads
    spinner.start();

    ros::ServiceServer service1 = n.advertiseService("FFPlan", srvPlan);
